...
postData(getChannelInfo + querys.channel).then((data) => {
    ...
    function sub_list_load(){
        const list_userName = document.querySelector('.profileInfo img').getAttribute('src');
        const list_userImageSrc = document.querySelector('.profileInfo div p').textContent;  
...
function addSubscribe() {
    ...
    if (subscribeButton.getAttribute('data-is-subscribed') === 'true') {
            ...
            var subHtml = `<a href="./7Channel.html?channel=${userName}">
            <span class="sidebar-text">
...
function filterVideos() {
  ...
  searchDayElement.style.fontWeight = 'bold';
...
const list_userName = document.querySelector('.profileInfo div p').textContent;
const list_userImageSrc = document.querySelector('.profileInfo img').getAttribute('src');

// 사이드바에 추가할 HTML 변수 지정
var subHtml = `<a href="./7Channel.html?channel=${list_userName}">
        <span class="sidebar-text">
            <img src="${list_userImageSrc}" alt="" />${list_userName}
        </span>
        </a>`  ;
...
if (sub_list === null || sub_list === undefined || Array.isArray(sub_list) && sub_list.length === 0) {        
    ...
    const normalizedArray = sub_list.map(normalize);       
    if (normalizedArray.includes(normalize(subHtml.trim()))) {        
        // SUBSCRIBES 회색으로 변경하고 사용자 정의 속성 (구독했음을 표시하는 속성) 추가
        subscribeButton.setAttribute('data-is-subscribed', 'true');
        subscribeButton.querySelector('img').style.filter = "grayscale(100%)";
    }
